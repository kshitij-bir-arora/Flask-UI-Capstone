...
def index():
    ...
    # Statistik
    top_category = pd.crosstab(index = df2['Category'],columns = 'Jumlah').reset_index().sort_values('Jumlah',ascending=False)
    rev_table = df2.groupby(['Category','App']).agg({'Reviews':'sum', 'Rating':'mean'}).sort_values('Reviews',ascending=False).reset_index().head(10)
    ...
    type_download = pd.crosstab(index = df2['Type'],columns = 'Frequency').reset_index()
    X = type_download['Type'].values
    Y = type_download['Frequency'].values
    fig = plt.figure(figsize=(5,5))
    fig.add_subplot()
